...
export class ChartService {
  ...
  private url = this.baseUrl + '/samples/charts';
  ...
  getSampleForMeasurement(measurementDetails: MeasurementDetail, valuePerPercentile: ValuePerPercentile) {
    ...
    console.log(params);
    let url = this.baseUrl +`/samples/charts/result/${measurementDetails.measurementType}/${measurementDetails.childSex}`;
    if (measurementDetails.measurementType === 'WEIGHT_FOR_LENGTH' || measurementDetails.measurementType === 'WEIGHT_FOR_HEIGHT'){
      url  = this.baseUrl +`/samples/charts/result/WEIGHT_FOR_LENGTH_HEIGHT/${measurementDetails.childSex}`;
    }
  ...
  getSampleForMeasurementWithBirthDate(measurementDetails: MeasurementDetail, valuePerPercentile: ValuePerPercentile, birthDate: string) {
    ...
    console.log(params);
    const url = this.baseUrl +`/samples/charts/result/${measurementDetails.measurementType}/${measurementDetails.childSex}`;
  ...
  getGeneralChart(data): Promise<ChartData> {
    let url = this.baseUrl + `/samples/charts/${data.measurementType}/${data.childSex}`;
    if (data.measurementType === 'WEIGHT_FOR_LENGTH' || data.measurementType === 'WEIGHT_FOR_HEIGHT'){
      url = this.baseUrl + `/samples/charts/WEIGHT_FOR_LENGTH_HEIGHT/${data.childSex}`;
    }
    const params = new URLSearchParams();
    console.log(data);
